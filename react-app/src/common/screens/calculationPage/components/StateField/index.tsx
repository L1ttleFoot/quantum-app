...
const StateField = () => {
    ...
    useEffect(
        () => {
            setStates(states.map(item=>({...item, value: [...new Array(freedomDegrees).fill('')]})))
        },
        // eslint-disable-next-line react-hooks/exhaustive-deps
        [freedomDegrees],
    );

    const handleChangeState = (index1: number, index2: number) => (event: React.ChangeEvent<HTMLInputElement>) => {
        setStates(states.map((item, index) => index===index1 ? ({...item, value: item.value.map((item, indexItem) => index2 === indexItem ? event.target.value : item)}) : item))
    };

    const handleChangeStateAdd = () => () => {
        setStates([...states, {index: states[states.length-1].index+1, value: [...new Array(freedomDegrees).fill('')]}])
    };
                        ...
                        <Stack style={{position:'relative'}} key={indexState} direction={'row'} alignItems={'center'}>
                            ...
                            {state.value.map((item, indexValue) =>
                                <TextField
                                    ...
                                    size={'small'}
                                    style={{ margin: 10 }}
                                    label={<span>n<sub>{indexValue+1}</sub></span>}
                                    value={item}
